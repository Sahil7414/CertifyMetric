...
Decision:
Expanded from a single hardcoded category (NAWI Class III) to **26** Legal Metrology categories, seeded in two tranches in `server/scripts/seedDemoUsers.js`.

First tranche (the high-volume consumer-facing instruments): NAWI (with accuracy class I/II/III/IIII as a spec field), Automatic Weighing Instruments, Weighbridges, Fuel Dispensing Pumps, CNG/LPG/LNG/Hydrogen Dispensers, Water Meters, Energy Meters, Gas Meters, Length Measures, Volumetric/Capacity Measures.

Second tranche (the remaining families that appear in the Legal Metrology (Approval of Models) Rules, 2011 schedule and in state verification/stamping fee schedules): Commercial Weights, Precision/Carat (Bullion) Weights, Beam Scale / Counter Machine, Spring Balance, Crane / Hanging Scale, Load Cell (component), Weighing Indicator (component), LPG Cylinder Filling Machine, Taximeter / Auto-Rickshaw Fare Meter, Bulk Liquid Flow Meter, Road Tanker Capacity Measure, Static Storage Tank, Dip Rod / Ullage Device, Textile Length Measuring Machine, Clinical Thermometer, Sphygmomanometer.

Why these 16 and not some other number: there is no single canonical count. The Approval-of-Models schedule runs roughly 25–35 entries, while state fee schedules enumerate 50+ line items only because they split one instrument family across size/capacity bands (a separate fee row per weighbridge tonnage, per weight denomination). We model the *family* as the category and push the band into `spec_schema`, so 26 families is the right granularity — not an under-count.

Two categories are **components**, not complete instruments: Load Cell and Weighing Indicator carry their own model approval and are verified as a matched pair with the instrument they are fitted into. Three categories are verified as a **batch** rather than per-unit (Commercial Weights, Precision Weights, Clinical Thermometers) — their `spec_schema` has a "Number of Pieces Presented" field for this. Taximeter is the only category whose `measurement_type` is not a physical quantity (`FARE`) — verification checks the programmed fare table and the distance/waiting-time computation. `measurement_type` now spans MASS, VOLUME, ENERGY, LENGTH, FARE, TEMPERATURE, PRESSURE.
...
The registration form (`AddInstrumentModal.jsx`) fetches categories via `GET /api/instrument-categories` and renders itself entirely from the selected category — `spec_schema` says *which* fields to collect, and a second column, `InstrumentCategory.form_meta`, says *how to present them*:

- `icon` — the drawer header glyph (a taxi for a taximeter, a barrel for a storage tank).
- `spec_section` — heading for the category's field group ("Fare Meter Configuration", not "Technical Specifications & Capacity").
- `capacity_fields` — whether to show the Max/Min/Interval(e) trio. This replaced a hardcoded `selectedCategoryId === 'CAT_NAWI_III'` check; only NAWI sets it.
- `serial_label` / `serial_hint` — a batch of weights has a *set number*, a storage tank has a painted *tank number*, a road tanker has a *chassis number*. Labelling all of these "Device Serial Number" was simply wrong.
- `manufacturer_placeholder` / `model_placeholder` / `location_label` / `location_placeholder` — realistic per-category examples.
- `notice` — an amber strip for categories registered unusually: stamped as a batch, approved as a component, or tied to a vehicle.

`form_meta` lives in the database, not in the React component, so adding a statutory category stays a pure seed change — consistent with the "configurable rather than hard-coded" principle in SIH26036_MASTER_CONTEXT.md. The component carries a `DEFAULT_FORM_META` fallback so a category seeded without it still renders sanely. The category `<select>` is grouped with `<optgroup>` by `measurement_type` (Mass & Weighing, Volume & Flow, Length, Electrical Energy, Temperature, Pressure, Fare Computation); anything with an unrecognised `measurement_type` falls into an "Other" group rather than silently disappearing.

**Server-side validation** (`POST /api/instruments`) treats `spec_schema` as the authority, not the client: required fields must be present, `select` values must be members of that field's `options`, `number` fields must be positive, and any key the category does not declare is stripped before the write. The last one matters because `Instrument.specs` is Mongoose `Mixed` — without it, switching category mid-form could persist the previous category's fields onto the record (a water meter carrying a `platform_length_m`). The Max/Min/Interval(e) columns are only accepted when the category's `form_meta.capacity_fields` is true, and are required when it is.

**Data provenance honesty check:** NAWI classes, weighbridge/dispenser/water-meter/gas-meter sub-types and size ranges are sourced from real OIML/ISO/BIS documentation (sources: OIML R76, R61, R106, R117; ISO 4064; EN 1359 — see chat history). Second-tranche sub-types, denominations and ranges follow the corresponding documents (OIML R111 weights, R60 load cells, R21 taximeters, R117 flow meters, R71/R80 tanks; IS 3784 clinical thermometers, IS 3390 sphygmomanometers). The `mpe_rules`/`checklist_schema` for every category OTHER than NAWI are simplified placeholders (generic seal/tamper checklist, no MPE table) — NOT transcribed from a verified primary statutory table. Energy meter accuracy classes (0.2S/0.5S/1/2) are industry-standard IEC 62053 classes, not yet checked against a specific Indian statutory schedule.

Status:
Resolved (registration) — Open sub-items:
1. Per-category MPE tables need real domain validation before the Verification Workspace could meaningfully test anything beyond NAWI. The workspace itself is still NAWI-only (zero/min/¼/½/max load test matrix doesn't apply to a water meter or length tape) — extending it to other categories is a separate, larger task not yet started.
2. Batch-verified categories (weights, thermometers) and component categories (load cell, indicator) are registrable and labelled correctly, but the downstream application/verification flow still treats every instrument as a single complete unit — a certificate is issued per record, not per piece in the batch, and a load cell is not yet linked to its host instrument. Fine for the demo, wrong in substance.
3. `form_meta` currently drives labels, placeholders, icon and section headings. It does NOT yet drive conditional fields *within* a category (e.g. a mercury sphygmomanometer and a digital one ask the same questions). If that becomes necessary, the extension point is a `show_if: {key, equals}` on a `spec_schema` entry.
