...
export function seedDemoUsers() {
  ...
  // 1. Ensure required schema columns and tables exist
  try {
  ...
  // 2. Upsert each demo account with hashed password
  const now = new Date().toISOString();

  for (const acc of DEMO_ACCOUNTS) {
    const hashed = hashPassword(acc.password);

    const existing = db.prepare('SELECT id FROM users WHERE id = ? OR LOWER(email) = LOWER(?)').get(acc.id, acc.email);
  ...
  // 3. Generate DEMO_CREDENTIALS.md in project root for local development reference
...
> Passwords are encrypted in SQLite using Node.js \`scrypt\` algorithm.
...
4. To re-seed or reset passwords at any time, run:
   \`\`\`bash
